...
const PRIMITIVES: ComponentDef[] = [
  {
    ...
    snippet: `<div class="p-6 rounded-2xl bg-zinc-900/60 border border-zinc-800 text-white space-y-3">
  ...
  },
  {
    ...
    snippet: `<section class="py-16 px-6 max-w-6xl mx-auto space-y-6">
...
export default function BasicComponents({ onInsert, selectedElement }: BasicComponentsProps) {
  ...
  return (
    <div className="h-full flex flex-col bg-[#0e0e0e] text-white">
      {/* Category Pills */}
      <div className="p-3 border-b border-[#222] space-y-2.5">
        <div className="flex items-center justify-between">
          <span className="text-xs font-semibold uppercase tracking-wider text-zinc-400">Insert Elements</span>
          <div className="flex items-center gap-1 bg-[#181818] p-0.5 rounded-lg border border-[#2a2a2a] text-[11px]">
            <button
              onClick={() => setPosition("after")}
              className={`px-2 py-0.5 rounded cursor-pointer transition-colors ${
                position === "after" ? "bg-[#282828] text-white font-medium" : "text-zinc-400 hover:text-white"
              }`}
            >
              After
            </button>
            <button
              ...
              className={`px-2 py-0.5 rounded cursor-pointer transition-colors ${
                position === "inside" ? "bg-[#282828] text-white font-medium" : "text-zinc-400 hover:text-white"
              }`}
            >
              Inside
            </button>
        ...
        <div className="text-[10px] text-zinc-400 flex items-center justify-between px-1 pt-1">
          <span>Drag into any container, or click to insert</span>
        </div>
      </div>
